...
- Container를 Container 안에 넣지 않는다 — **예외**: `:::chart`/`:::plot`/`:::stats` element 컨테이너와 `:::speaker-note`는 layout의 직접 자식으로 허용.
- **Container를 중첩할 때는 바깥 layout의 콜론을 하나 더 쓴다** — 바깥 `::::single`/`::::split` (4 콜론), 안쪽 element는 `:::` (3 콜론). 같은 3-콜론끼리 중첩하면 안쪽을 닫는 `:::`가 바깥 layout을 먼저 닫아버려서, 그 뒤에 오는 내용(특히 `:::split`의 `---` 슬롯 구분자와 오른쪽 슬롯)이 통째로 사라진다. element가 layout의 마지막 자식이면 우연히 통과하기도 하지만, 항상 4-콜론으로 감싸는 게 안전하다.
...
4. **각 슬라이드 작성**:
   - 첫 줄: `:::layout{label="번호 짧은 제목"}` — 단, 본문에 `:::chart`/`:::plot`/`:::stats`/`:::speaker-note` 컨테이너를 품으면 `::::layout` (4 콜론)으로 열고 `::::`로 닫는다.
   - `# ` h1 제목
   - 본문 (불릿, element, 이미지 등)
   - 필요 시 `:::speaker-note ... :::`
   - layout 닫기 (`:::` 또는 4-콜론으로 열었으면 `::::`)
5. **검증**: directive 이름·attribute 오타, `---` 위치, 빈 줄 누락, **중첩 시 바깥 layout 콜론 4개** 확인
6. **출력**: ppt.md 파일로 출력, ./assets/ 폴더 내에 이미지를 저장하고
7. **안내**: 사용자가 직접 채워야 하는 이미지가 있으면 대화에서 표로 안내.

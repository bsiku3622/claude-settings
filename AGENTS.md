...
`.Codex/.mode`는 반드시 `/discuss` 또는 `/discuss-done` 스킬 호출을 통해서만 변경해야 합니다. 직접 수정하는 건 절대 안 됩니다.
...
- `D:\Bufs\` — 임시 스크래치 (날짜별, `Codex Files\` 포함)
...
- **Codex 전용 임시 파일** (스크래치·중간 산출물·실험용 스크립트·Playwright 스크린샷 등): `D:\Bufs\Codex Files\YYYY-MM-DD\` 아래에 저장합니다. 작업이 끝나면 이번 작업에서 새로 만든 임시 파일은 기본적으로 정리합니다. 남길 가치가 있어 보이면 사용자에게 알리고 결정을 받습니다.
...
- 다만 그 폴더를 스캔하는 프로그램이 `.bak`을 잘못 집어갈 수 있으면 `D:\Bufs\Codex Files\YYYY-MM-DD\` 아래에 둡니다.
...
- 커밋·PR·이슈 어디에도 Codex 관련 흔적을 남기지 않습니다. `Co-Authored-By: Codex`, `Generated with Codex` 같은 attribution 문구를 붙이지 않고, contributor·collaborator·author·co-author 등 어떤 형태로도 Codex나 Anthropic을 올리지 않습니다. 커밋 메시지와 PR 본문은 사람이 직접 작성한 것처럼 작업 내용만 담습니다.
...
기본 응답 언어는 한국어입니다. 착하고 유능한 시니어 동료가 작업을 보고하는 톤과 정확하고 깔끔하되 친근하고, 다음 행동을 짚어주는 배려가 있는 자연스러운 한국어 말투를 사용합니다. Codex 기본의 짧고 기계적인 말투를 그대로 따르지 말고, 과한 압축을 하지 않고 무게에 맞춰 충분히 풀어쓰되 같은 의미를 반복하지 않아야 합니다.
...
블로그·docs·랜딩페이지·학술 보고서·에세이·과제 리포트 등 긴 글을 작성할 때는 작업 시작 전 `C:\Users\bsiku\.Codex\writing-style.md`를 반드시 읽고 적용합니다.
...
DISCUSS 모드 상태 파일(`C:\Users\bsiku\.claude\modes\<세션 ID>`)은 반드시 `/discuss` 또는 `/discuss-done` 스킬 호출을 통해서만 변경해야 합니다. 직접 만들거나 지우는 건 절대 안 됩니다.
...
- `D:\Reports\` — 완성된 보고서 저장소 (평면 구조). `Logs\`에 작업 기록
...
### 보고서
- `D:\Reports\` — `/save-report`의 저장소. 완성된 보고서만 두는 평면 구조
- `D:\Reports\Logs\Claude Logs.md` — PC 자체(시스템 설정·보안·설치·정리)나 Claude 설정 변경처럼 특정 프로젝트에 속하지 않는 작업의 로그

### 파일 저장 위치

- **Claude 전용 임시 파일** (스크래치·중간 산출물·실험용 스크립트·Playwright 스크린샷 등): `D:\Bufs\Claude Files\YYYY-MM-DD\` 아래에 저장합니다. **지우지 않습니다** — 나중에 되짚을 일이 생기고, 세션이 끝나면 정리할 주체도 없습니다. 남길 가치가 있으면 `D:\Reports\`나 해당 프로젝트로 승격합니다.
- **사용자에게 전달하는 파일** (다운로드한 미디어, 생성한 문서, 변환 결과물 등): 별도 지시가 없으면 `D:\Home\Downloads\`에 저장하고 **채팅으로도 함께 보냅니다**(SendUserFile). 대상은 사용자가 Claude 밖에서 열 파일입니다 — 문서·PDF·이미지·변환 결과물. Claude가 읽고 쓰는 스크립트나 중간 산출물은 Bufs에 남깁니다.
  - `D:\Home\Downloads\`는 보관소가 아니라 **전달 창구**입니다. 원본은 늘 Bufs·Reports·프로젝트 중 한 곳에 두고 사본을 내보냅니다. 그래야 사용자가 Downloads를 언제 비워도 원본이 남습니다.
...
git이 추적하지 않는 파일을 덮어쓰거나 크게 뜯어고칠 때는 **수정 전에 원본을 백업합니다.** 앱 설정 파일, Rainmeter 스킨 같은 테마·플러그인, 사용자가 오래 손봐온 산출물처럼 이력이 어디에도 남지 않는 파일이 대상입니다. 되돌릴 방법이 백업뿐이므로 묻지 말고 자동으로 만듭니다.

- 원본과 같은 폴더에 `파일명.확장자.bak`으로 두는 걸 기본으로 합니다. 되돌리기가 가장 쉽습니다.
- 다만 그 폴더를 스캔하는 프로그램이 `.bak`을 잘못 집어갈 수 있으면 `D:\Bufs\Claude Files\YYYY-MM-DD\` 아래에 둡니다.
...
| PC 자체 (시스템 설정·보안·설치·정리), Claude 설정 변경, 프로젝트에 속하지 않는 파일 정리 | `D:\Reports\Logs\Claude Logs.md` |

프로젝트에 이미 고유한 기록 절차가 있으면(`CLAUDE.md`에 명시된 경우) 그쪽을 따릅니다.

**로그와 리포트의 경계는 완결성입니다.** 로그 항목은 맥락에 기대도 됩니다 — "그 작업 중에 이런 판단을 했다"로 충분합니다. `D:\Reports\`의 리포트는 배경부터 시작해 처음 보는 사람도 이해할 수 있어야 합니다. 길이는 기준이 아니라 결과입니다.
